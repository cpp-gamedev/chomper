...
MainMenu::MainMenu(gsl::not_null<Engine*> engine) : m_engine(engine) {
...
void MainMenu::tick(kvf::Seconds const dt) {
	swingMainText(dt);
...
void MainMenu::render(le::IRenderer& renderer) const {
	m_mainText.draw(renderer);
}

void MainMenu::swingMainText(kvf::Seconds const dt) {
